...
def setup_training(self):
    ...
    self.max_history_length = 2 # Max length of history to detect loops
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    # Got closer to chest
    if old_coin_distance == np.inf and old_dangerzone[self_pos] == 0 and new_dangerzone[self_pos] == 0 and not e.WAITED in events:      
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    # Store the model
    with open("Res_DuelDQN.pt", "wb") as file:
        self.model.save(file)

    scorefile = os.path.join("scores", "Res_DQL_3B_1L_against_rule_based_5000.txt")

    # Append the score to the file
    with open(scorefile, "a") as f:
        f.write(f"{score},{last_game_state['step']},{self.bomb_counter},{self.invalid_move_counter},{round(self.model.epsilon,2)}\n")
...
def reward_from_events(self, score, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        e.INVALID_ACTION: -50,
        WENT_INTO_DANGERZONE: -40, 
        USELESS_BOMB: -35,
        GOT_FURTHER_AWAY_FROM_COIN: -35,
        SPEND_TIME_IN_DANGER_ZONE: -30, 
        GOT_FURTHER_AWAY_FROM_CHEST: -25,
        e.WAITED: 0,
        e.MOVED_DOWN: 0,
        e.MOVED_LEFT: 0,
        e.MOVED_RIGHT: 0,
