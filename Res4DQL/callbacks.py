...
from .models import Res_DuelingDQNAgent as Agent
...
def setup(self):
    ...
    scorefile = os.path.join("scores", "Res4DQL.txt")  # File to store the scores
    ...
    with open(scorefile, "a") as f:
        f.write("Score,Steps,Bombs_placed,Valid_moves_quota, Kills, Epsilon\n")

    if not os.path.isfile(datafile):  # If there is a file to load from, do it
...
def act(self, game_state: dict) -> str:
    ...
    if np.random.rand() <= self.model.epsilon: ## very naive epsilon greedy approach
            self.logger.info(f"Model is Exploring.{self.model.epsilon}")
        ...
        _, _, path_to_objective = dijkstra_shortest_path_to_targets(game_state, ['coins'])
        ...
        if not path_to_objective:
            ...
            if len(path_to_objective) > 2:
                path_to_objective = path_to_objective[:-2]
            else:
                path_to_objective = []
    ...
    while pq:
        ...
        for dx, dy in directions:
            ...
            if 0 <= nx < height and 0 <= ny < width and not visited[nx, ny]:
                ...
                if field[nx, ny] == 0: # 0 means free space 
